...
The full geometrical arena of an $SU(N)$ gauge theory over $\mathbb{R}^{4}_{\infty}$ is that of the universal bundle $[ SU(N)\times\mathcal{G} ] \hookrightarrow ( P \times \mathcal{A} ) \rightarrow ( \mathbb{R}^{4}_{\infty} \times\mathcal{A}/\mathcal{G} )$~\cite{baulieu1985a,baulieu1988a}. The smooth  $(N^2+3)$-manifold $P$ is part of the more commonly known principle bundle structure $SU(N) \hookrightarrow P \rightarrow \mathbb{R}^{4}_{\infty}$, together with its adjoint bundles, $ \mathrm{Ad} P \equiv P \times_{SU(N)} SU(N) $ and $ \mathrm{ad}P \equiv P \times_{SU(N)} \mathfrak{su}(N)$. The space of all smooth $SU(N)$-connections on $ P $ is $ \mathcal{A} \equiv C^{ \infty } ( J^1 P ) $, where $ J^1 P $ is its 1st jet bundle. The space of all smooth gauge transformations is $ \mathcal{G} \equiv C^{ \infty } ( \mathrm{Ad}P ) $, and $ C^{ \infty } ( \mathrm{ad} P ) $ is its Lie algebra.

A gauge field $A(x)$ is defined as an element in $C^\infty ( \mathrm{ad}P \otimes \bigwedge^1\mathbb{R}^{4}_{\infty} )$ --- a $ \mathrm{ad} P $-valued 1-form field on $\mathbb{R}^{4}_{\infty}$ ---, which results from a pullback to spacetime of an $SU(N)$-connection living on $P$. Similarly, a universal gauge field $\tilde{A}(x)$ is defined as the result of a pullback to $\mathbb{R}^{4}_{\infty} \times \mathcal{A}/\mathcal{G}$ of a universal $[SU(N) \times \mathcal{G}]$-connection living on $P \times \mathcal{A}$. And, it can be written as
\begin{equation}
  \tilde{A} = A + c \;,
\end{equation}
...
The exterior derivative $\tilde{d}$ on $\mathbb{R}^4 \times \mathcal{A}/\mathcal{G}$ can also be \textquote{decomposed} with respect to this product, resulting in
\begin{equation}%
...
\end{equation}
contains, precisely, the Donaldson polynomials first evaluated in the seminal works of S.~K.~Donaldson~\cite{donaldson1983a,donaldson1990a}, and E.~Witten~\cite{witten1988d} for $N=2$.
...
\begin{equation}%
  ...
  = 8 \pi^2 k \;,
\end{equation}
where $ k \in \mathbb{Z} $ is the so-called \textit{instanton number}\footnote{The instanton number is generally non-vanishing on $\mathbb{R}^{4}_{\infty}$. It is well-known that $\tr \left(F^2\right)$ is a $d$-cycle, and Poincaré lemma implies it is a $d$-boundary. In fact, $\tr \left(F^2\right) = \tr \left( d \omega^{(3)} \right)$, where $\omega^{(3)} \equiv AdA+\tfrac{2}{3}A^3$ is the famous Chern-Simons 3-form. However, this is a local statement, and the 4th de Rham cohomology group, $ H^4_{\text{dR}} \left( \mathbb{R}^{4}_{\infty}\right) \sim H^4_{\text{dR}} \left( S^{4}\right) \sim \mathbb{Z} $, obstructs this statement to be globally true.} of the gauge field $A$. More precisely, $k$ is the 2nd Chern number of the $SU(N)$-bundle over $\mathbb{R}^{4}_{\infty}$, and on which the principal connection associated with $A$ was originally defined. This is an important distinction because $k$ classifies these bundles. Gauge fields (modulo gauge transformations) are in one-to-one correspondence with them (modulo bundle isomorphisms). In this sense, TYM is explicitly a topological field theory, and~\eqref{eq:tym-action} is explicitly a smooth invariant.
