...
\begin{subequations}%
  ...
  \end{align}
\end{subequations}
and it should be noticed that $s$ is nilpotent only up to a gauge transformation generated by $\phi$. In other words, in the Cartan model, $s$ is nilpotent only in the subspace of gauge invariant polynomials.
...
\begin{subequations}%
  ...
  \begin{align}%
    \bar{S}_{\text{aFH}}[A, \psi, \varphi, \xi, \eta, b] & = \bar{s} \int \tr \left\{ D \xi \star D\varphi + \mathrm{ad}_{\Phi_0} D \xi \star A + \xi ( \Phi_0 + \varphi ) \left[ m^2 \right. + \right.                                                      \nonumber \\
                                                         ...
                                                         & \left. - \left. 2g \left(B_0 \varphi + b \Phi_0 + b \varphi \right) \xi \eta + g \xi \eta \xi \eta \vphantom{m^2} \right] \star \mathds{1} \right\} \;, \label{eq:afh-eff-action-full}
  \end{align}
\end{subequations}
where $\bar{S} \equiv S_{\text{aFH}} [\Phi_0 + \varphi, B_0 + b]$, and $\bar{s}$ is given by~\eqref{eq:afh-bar-brst}. In particular, its quadratic part
\begin{align}%
...
No massless fermions were generated in the effective action above, leading us to conclude that the BRST symmetry is unbroken. It is trivial but pedagogically helpful to mention that, in this case, the effective BRST around $(B_0, \Phi_0)$,
\begin{subequations}%
  ...
  \end{align}
\end{subequations}
defined to preserve it, $\bar{s} (B_0, \Phi_0) \equiv (0, 0)$, remains nilpotent and trivially isomorphic to the original BRST in~\eqref{eq:cartan-model-afh-brst}. In fact, in this neighborhood,~\eqref{eq:cartan-model-afh-brst} assumes the form
\begin{subequations}%
  ...
  \end{align}
\end{subequations}
Unsurprisingly,~\eqref{eq:afh-eff-action-full} can be written as the $\bar{s}$-boundary~\eqref{eq:afh-eff-action-exact}, which automatically makes it a $\bar{s}$-cycle.

The \textquote{spontaneous} gauge symmetry breaking that just occurred must not be interpreted as a traditional physical process. Before and after the Higgs phase, bulk energy functional remains trivial due to the BRST stability. The contribution given by~\eqref{eq:afh-action} is
\begin{subequations}%
  \label{eq:afh-energy}
  \begin{align}
  ...
  \end{align}
\end{subequations}
where $\mathcal{L}_{\lambda} \equiv d \lambda \rfloor + \lambda \rfloor d$ is the Lie derivative along $\lambda$, $\mathfrak{L}_{\text{aFH}}$ is the Lagrangian 4-form in~\eqref{eq:afh-action-full}, and the vertices containing $c$ can be eliminated via gauge fixing\footnote{The ghost field $c$ resurrected because the canonical energy-momentum 3-form of a non-Abelian gauge theory is generally not a gauge invariant polynomial. Thus, we had to change back to the BRST model. Nonetheless, if temporal gauge conditions are assumed ($\lambda \rfloor A = \lambda \rfloor \psi =0$), then one can show that $\mathcal{L}_{\lambda}c$ identically vanishes in the bulk. }. In this sense, every physical field configuration in the bulk is part of the vacua moduli. The symmetry breaking did not \textquote{occur} because $(B_0, \Phi_0)$ is energetically favored. In fact, it is not sensible to say anything non-topological \textquote{occurred} in a topological field theory. Thus, the effective action~\eqref{eq:afh-eff-action} should be pragmatically interpreted as a representation of~\eqref{eq:afh-action-full} in the neighborhood of a non-trivial point $(B_0, \Phi_0)$ of the vacua moduli.

A final comment is about the possible presence of solitonic degrees of freedom at the Higgs phase. In the traditional Yang-Mills-Higgs model, the gauge vacua are the moduli of flat connections, homotopically equivalent to a point. Thus, the homotopy of the adjoint Higgs sector alone, here $\pi_2 (SU(N)/{U(1)}^{m-1}) \sim \mathbb{Z}^{m-1}$, is sufficient to determine that $m-1$ independent topological solitons of spatial co-dimension 3 will localize in $\mathbb{R}^4$. A Riemannian or Lorentzian structure would then imply these are vertices or monopoles, respectively. However, in TYM the gauge vacua is the entire gauge moduli $\mathcal{M}$, which is not homotopic to a point and cannot be disregarded. Contributions from all these gauge vacua can potentially destabilize Higgs sector solitons, and \textit{vice versa}. Thus, a careful analysis of the complete moduli $[\mathcal{A}_{\mathcal{H}} \times C^{\infty} (\mathrm{ad}P_{\mathcal{H}})] / \mathcal{H} $ is unavoidable, and out of our scope.
\end{document}
