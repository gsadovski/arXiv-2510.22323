...
A final comment is about the possible \textquote{generation} of topological solitons. In the traditional adjoint Yang-Mills-Higgs model, the gauge vacua are the moduli of flat connections, homotopically equivalent to a point. Thus, the homotopy of the Higgs sector alone, here $\pi_2 (SU(N)/{U(1)}^{m-1}) \sim \mathbb{Z}^{m-1}$, is sufficient to determine that $m-1$ independent topological solitons of spatial co-dimension 3 localize in $\mathbb{R}^4$. A Riemannian/Lorentzian structure would then imply these are vertices/monopoles. However, in the topological case the gauge vacua is the entire gauge moduli $\mathcal{M}$, which is not homotopic to a point, and cannot be disregarded. Contributions from all these vacua can potentially destabilize solitons that would otherwise exist in the isolated Higgs sector, and \textit{vice versa}. Thus, a careful analysis of the complete moduli $[\mathcal{A}_{\mathcal{H}} \times C^{\infty} (\mathrm{ad}P_{\mathcal{H}})] / \mathcal{H} $ is unavoidable when investigating possible solitonic degrees of freedom.
\end{document}
