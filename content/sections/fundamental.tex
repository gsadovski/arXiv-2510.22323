...
\section{TYMH theories with BRST instability}%
\label{sec:fund-higgs}

Now, let us consider the Fujikawa-Higgs (FH) doublets in the fundamental representation of $SU(N)$. For that, we introduce the associated vector bundle $E \equiv P \times_{SU(N)} \mathbb{C}^N$, its endomorphism bundle $\mathrm{End}(E) \equiv P \times_{SU(N)} \mathrm{End}(\mathbb{C}^N)$, and automorphism bundle $\mathrm{Aut}(E) \equiv P \times_{SU(N)} \mathrm{Aut}(\mathbb{C}^N)$. The group of gauge transformations $\mathcal{G}$ has faithful representation in $\mathrm{Aut}(E)$, and its Lie algebra $\mathrm{Lie(\mathcal{G})}$ has in $\mathrm{End}(E)$. The TYM fields $(A, c, \psi, \phi)$ are now elements in $C^{\infty}(\mathrm{End}(E) \otimes \bigwedge \mathbb{R}^4_{\infty} )$, while the FH doublets $(\Phi, \eta, \xi, B)$ are elements in $ C^{\infty}(E \otimes \bigwedge^0 \mathbb{R}^4_{\infty} )$. Again, their grading can be found at Table~\ref{tab:gradings}.
...
In full analogy with the adjoint model, we define the Fujikawa-Higgs sector of \textquote{topological Yang-Mills-Higgs} (TYMH) theories via the action functional
\begin{subequations}%
  \label{eq:ffh-action}
  \begin{align}%
  ...
  \end{align}
\end{subequations}
where $\Xi$ is shorthand for $A, \psi, \Phi, \eta, \xi, B, {\Phi}^{\dagger}, {\eta}^{\dagger}, {\xi}^{\dagger}, {B}^{\dagger}$. However, our analysis is short-lived because the resulting bosonic potential,
\begin{equation}%
  \label{eq:ffh-potential}
  V_{\text{FH}} ( \Phi, B, {\Phi}^{\dagger}, {B}^{\dagger} ) = \re \left[m^2 B^{\dagger} \Phi + g {( B^{\dagger} \Phi )}^2\right] \;,
\end{equation}
is not bounded from below (or above). Assuming $g \neq 0$, one can easily show that the only critical point,
\begin{subequations}%
  \label{eq:saddle-pts}
  \begin{align}
  ...
  \end{align}
\end{subequations}
is a saddle. Thus, there is no spontaneous gauge or BRST symmetry breaking since there is no analog of~\eqref{eq:afh-eff-action} and~\eqref{eq:afh-bar-brst}. Stated more clearly, we conclude that, in the fundamental representation of $SU(N)$, and bosonic potential given by~\eqref{eq:ffh-potential}, TYMH theories does not admit representation in terms of massive gauge fields, solitons, nor does it admit a non-topological phase.

\subsection{Realified theory}%
...
The gauge symmetry breaking pattern differs from usual Higgs model from particle physics due to realification. Let us first recall the standard Higgs, with non-trivial vacua $\Phi_0$, and stabilizer $\mathfrak{h}_0 = \left\{ X \in \mathfrak{su}(N) \; ; \; X \Phi_0 = 0\right\} $ in $ \mathcal{V}_0 $. The condition $X \Phi_0 = 0$ forces the 1st row, and 1st column of $X$ to vanish, making it effectively of rank $N-1$. This is the usual symmetry breaking pattern: in the trivial vacuum neighborhood, $\mathfrak{h}_0 \sim \mathfrak{su}(N)$, while in the neighborhood of $\Phi_0$, we have $\mathfrak{h}_0 \sim \mathfrak{su}(N-1)$. Consequentially, $2N-1$ generators are broken, resulting in $2N-1$ massive gauge bosons. Turning to realified Higgs, $\Phi_0$ has stabilizer $\mathfrak{h}_0 = \left\{ X \in \mathfrak{su}(N) \hookrightarrow \mathfrak{so}(2N) \; ; \; X \Phi_0 = 0 \right\}$, where $X$ has the form~\eqref{eq:realified-matrix}. Notice that if its 1st row, and 1st column vanish, so does its $(N+1)$-th row, and $(N+1)$-th column, making it effectively of rank $2N-2$. Therefore, realification changes the gauge symmetry breaking pattern from $\mathfrak{su}(N) \mapsto \mathfrak{su}(N-1)$ to $\mathfrak{so}(2N) \mapsto \mathfrak{so}(2N-2)$. Since $4N-3$ generators are broken, $4N-3$ gauge boson are massive. In fact, realification amplifies\footnote{Roughly analogous to how an embedding $S^{1} \hookrightarrow \mathbb{R}^3$ gives the circle freedom to knot itself, a realification embedding $SU(N) \hookrightarrow SO(2N)$ gives TYMH theories more gauge directions to break.} the gauge symmetry breaking, increasing the number of massive gauge bosons by $2N-2$.
...
\begin{subequations}% 
  ...
  \end{align}
\end{subequations}
In comparison to its adjoint analog~\eqref{eq:afh-bar-brst}, it has acquired shifts which indeed spoil the doublet and basic nature of the (effective) FH fields. It remains nilpotent (up to $\delta_{\phi}$), but it is no longer isomorphic to~\eqref{eq:real-brst} in the subspace of basic forms. In other words,~\eqref{eq:real-eff-brst} does not represent the Cartan model of the $\mathcal{G}$-equivariant cohomology of $\mathcal{A}$. More dramatically,~\eqref{eq:real-eff-action} is not a $\bar{s}$-cycle, automatically implying it also not a $\bar{s}$-boundary. The effective theory has lost every trace of BRST invariance. In particular, the dodge of the doublet theorem by the FH fields, and the presence of $\mathrm{g}$ metric-contaminated terms in~\eqref{eq:real-eff-action}, imply local (non-topological) degrees of freedom are now present and physically relevant in the bulk. In other words, in this region of the moduli, $\text{TYMH}_{\mathbb{R}}$ is no longer a topological field theory.

It is straightforward to verify that the energy functional $\mathbb{E}_{\text{TYMH}_{\mathbb{R}}}$, evaluated from~\eqref{eq:real-ffh-action} is bulk trivial, but $\bar{\mathbb{E}}_{\text{TYMH}_{\mathbb{R}}}$, evaluated from~\eqref{eq:real-eff-action}, is not. Again, due to the tree-level instability. Thus, the effective gauge vacua are already localized on the moduli of flat connections. Despite this, solitonic degrees of freedom remain quite rare occurrences. The reason is that $SO(2N)/SO(2N-2)$ are quotients diffeomorphic to Stiefel manifolds $V_2 (\mathbb{R}^{2N})$. Their non-trivial homotopy, $\pi_{2N-2} [ V_2 ( \mathbb{R}^{2N})] \sim \mathbb{Z} $, implies that only for $N=2$, we have a single topological soliton of spatial co-dimension 3 localizing in ${\mathbb{R}}_{\infty}^{4}$. A Riemannian or Lorentzian structure then manifests it as a vertex or monopole, respectively.
