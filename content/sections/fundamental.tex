...
\begin{subequations}%
  ...
  \end{align}
\end{subequations}
where $\Xi$ is shorthand for $A, \psi, \Phi, \eta, \xi, B, {\Phi}^{\dagger}, {\eta}^{\dagger}, {\xi}^{\dagger}, {B}^{\dagger}$. However, our analysis is cut short because the resulting bosonic potential,
\begin{equation}%
...
\end{equation}
where $\tr_{\mathbb{R}} (\hphantom{F}) \equiv 2\re [ \tr (\hphantom{F}) ] $ is the realified trace on $\mathrm{End}(\mathbb{C}^N_{\mathbb{R}})$.
...
The Higgs phase effective theories have lost every trace of topological BRST invariance --- the non-topological features hidden in the quantum regime of aTYMH theories, are here exposed at tree-level. In particular, the dodge of the doublet theorem by the FH fields, and the presence of $\mathrm{g}$ metric-contaminated terms in~\eqref{eq:real-eff-action}, together imply local (non-topological) degrees of freedom are now present and physically relevant in the bulk. In other words, in this region of the moduli, $\text{TYMH}_{\mathbb{R}}$ theories are clearly local field theories.
